...
class SimpleBrain:
    def __init__(self, w1=None, w2=None):
        # Вход (7) -> Скрытый (4) -> Выход (2)
        self.w1 = w1 if w1 else [[random.uniform(-1, 1) for _ in range(7)] for _ in range(4)]
        self.w2 = w2 if w2 else [[random.uniform(-1, 1) for _ in range(4)] for _ in range(2)]
    
    def predict(self, inputs):
        hidden = [math.tanh(sum(i * w for i, w in zip(inputs, row))) for row in self.w1]
        return [math.tanh(sum(h * w for h, w in zip(hidden, row))) for row in self.w2]
        
    def mutate(self):
        new_w1 = [[v + random.uniform(-0.1, 0.1) for v in r] for r in self.w1]
        new_w2 = [[v + random.uniform(-0.1, 0.1) for v in r] for r in self.w2]
        return SimpleBrain(new_w1, new_w2)
...
def main():
    ...
    while True:
        ...
        screen.blit(f_b.render("СИСТЕМА ЭВОЛЮЦИИ 3.5 (2-СЛОЯ)", True, (0, 255, 200)), (1270, 20))
        ...
        if selected and selected in shapes:
            pygame.draw.rect(screen, (20, 20, 40), (1265, 180, 320, 420), border_radius=10)
            ...
            for i, txt in enumerate(info): screen.blit(f_s.render(txt, True, selected.color), (1280, 195 + i*20))
            
            in_l = ["ВХОД: X", "ВХОД: Y", "ВХОД: HP", "УГОЛ Ц.", "ДИСТ Ц.", "ВРАГИ", "CONST"]
            hid_l = ["H1", "H2", "H3", "H4"]
            out_l = ["ПОВОРОТ", "СКОРОСТЬ"]
            
            # Отрисовка нейронки
            # Слой 1 -> Скрытый
            for i, l in enumerate(in_l):
                screen.blit(f_s.render(l, True, (120, 120, 120)), (1275, 330 + i*18))
                for h in range(4):
                    w = selected.brain.w1[h][i]
                    c = (0, 255, 100) if w > 0 else (255, 50, 50)
                    pygame.draw.line(screen, c, (1325, 338+i*18), (1430, 355+h*50), 1)
            
            # Скрытый -> Выход
            for h in range(4):
                screen.blit(f_s.render(hid_l[h], True, (180, 180, 180)), (1435, 350 + h*50))
                for j in range(2):
                    w = selected.brain.w2[j][h]
                    c = (0, 255, 100) if w > 0 else (255, 50, 50)
                    pygame.draw.line(screen, c, (1450, 358+h*50), (1510, 400+j*120), 1)
            
            for i, l in enumerate(out_l):
                screen.blit(f_s.render(l, True, (0, 200, 255)), (1515, 395+i*120))
